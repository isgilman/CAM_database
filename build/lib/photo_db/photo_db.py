...
def fill_list_pwys(df, thelist=photo_db.pull_data('the_list'), warnings=True, debug=False):
    ...
    Parameters
    ----------
    df : pandas DataFrame with relatively similar structure to that pulled 
        by pull_data('cam_db')
    thelist : pandas DataFrame with relatively similar structure to that 
        pulled by pull_data('the_list') (default = photo_db.pull_data('the_list'))
    warnings : print of taxa that exhibit strange combinations
        of pathway and dC13 values (bool; default = True)
    debug : print informative print statements for debugging (bool; default = False)
            ...
            if genus in templist.index:
                ...
                if len(listpwys)>1 and not photo_db.check_nan(row["dC13"]):
                    if row["dC13"]<-22.0:
                        ...
                        if debug: print("\tSome data: {}, {}, {}". format(genus, listpwys, row["dC13"]))
                        if len(listpwys)>1:
                            newpwy = np.random.choice(listpwys, size=1)[0]
                        else:
                            newpwy = listpwys[0]
                    elif row["dC13"]>-20.0:
                        # Remove C3 if dC13 is very positive
                        listpwys = list(set(listpwys)-set(['C3']))
                        if debug: print("\tSome data: {}, {}, {}". format(genus, listpwys, row["dC13"]))
                        if len(listpwys)>1:
                            newpwy = np.random.choice(listpwys, size=1)[0]
                        else:
                    ...
                    if not photo_db.check_nan(row["dC13"]):
                        if newpwy=='CAM' and row["dC13"]<-22.0 and warnings:
                            ...
                            print("WARNING: {} was resolved as {} with dC13={}". format(genus, newpwy, row["dC13"]))
                        elif newpwy=='C3' and row["dC13"]>-20.0 and warnings:
                            print("WARNING: {} was resolved as {} with dC13={}". format(genus, newpwy, row["dC13"]))
                if debug: print("\t>>>RESOLVED: {}, {}, {}". format(genus, newpwy, row["dC13"]))
            df.loc[index, "Pathway"] = newpwy
